...
namespace DAL.Core
{
    public class ModelAdapter : IModelAdapter
    {
        ...
        public async Task<DataSet> GetDataIntoDataSet(string cmdText)
        {


            try
            {
                return await ExecuteIntDataSet(cmdText);
        ...
        where T : class
        {
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    var fields = ModelAdapter.GetFields(reader);
        ...
        where T : class
        {
            return ModelAdapter.CreateModel<T>(data);
        }
        ...
        where T : class
        {
            ...
            {
                list.Add(ModelAdapter.CreateModel<T>(datum));
            }
            return list;
        }
        ...
        DataSet DataReaderToDataSetOptimize(IDataReader reader)
        {
            using (reader)
            {
                DataSet dataSet = new DataSet();
                if (reader != null)
                {
                    ...
                    do
                    {
                        // Create a new DataTable and specify its name
                        DataTable dataTable = new DataTable("Table" + (dataSet.Tables.Count + 1));
                        for (int i = 0; i < fieldCount; i++)
                        {
                            dataTable.Columns.Add(reader.GetName(i), reader.GetFieldType(i));
                        }
                        ...
                        // Use a SqlDataAdapter to fill the DataTable with data from the IDataReader
        ...
        private async Task<DataSet> ExecuteIntDataSet(string cmdText)
        {
            ...
            //try
            //{
            //    _connection = new OracleConnection(dbConnection.ConnectionString);
            //    _connection.Open();
        ...
        private async Task<List<Dictionary<string, object>>> GetAll(DbDataReader reader)
        {
            ...
            while (true)
            {
                ...
                list.Add(ModelAdapter.GetFields(list1, reader));
            }
            return list;
        }
        ...
        private static string MakeSearchQuery(GridParameter finder, out Dictionary<string, object> queryParams, string sql)
        {
            ...
            string str1 = ModelAdapter.MakeParameter(finder.Parameters, out queryParams);
